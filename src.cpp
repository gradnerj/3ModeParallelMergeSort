...
const unsigned int END_BLOCK_SIZE = 2048;
...
void n_is_odd(int sorted_block_size, int start_idx, int end_idx, int* arr, int * output){
...
void n_is_even(int sorted_block_size, int start_idx, int end_idx, int* arr, int * output){
...
int main(){
	...
	for(unsigned int i=0; i < N; i+= BLOCK_OF_SORTED_BLOCKS){
		...
		if(power_of_n % 2 == 0){
			n_is_even(sorted_block_size, start_idx, end_idx, arr, output);
		}else{
			n_is_odd(sorted_block_size, start_idx, end_idx, arr, output);
		}
	}
/*	
